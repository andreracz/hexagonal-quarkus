...
public class AccountRepositoryJdbcImpl implements AccountRepository {
    ...
    public List<Account> listAccounts() {
        return repository.createQuery("from AccountModel", AccountModel.class).getResultList().stream().map(AccountRepositoryJdbcImpl::toAccount).collect(Collectors.toList());
    }
    ...
    public static Account toAccount(AccountModel model) {
        return new Account(model.getClientId(), 
                            ...
                            model.getTransactionList()
                                    .stream()
                                    .map(AccountRepositoryJdbcImpl::toTransaction)
